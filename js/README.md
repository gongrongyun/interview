...
> ### 深拷贝与浅拷贝的区别
>
> > 在 js 中,数据类型有两类,一类是基本类型,另外一类就是引用类型.我们都知道 js 中的基本类型有 5 种(不包括 Symbol 类型),分别是`null`,`undefined`,`number`,`string`,`boolean`.除了这五种基本类型,其余都是引用类型,包括使用`Object`对基本类型做的包装出来的对象.在对基本类型的值的复制时,将会直接创建一个新的基本类型赋值给新的变量,修改新的变量不会影响原始值,而在对引用类型的值进行复制时,将会把该对象的引用(即内存地址)赋值给新的变量.这样修改新的变量时,也会反映在原始的值上.  
> > 因此,从上面的定义我们可以知道深拷贝就是将需要拷贝的对象中的引用类型的值也拷贝过去(开辟新的内存空间)而不是拷贝其引用,浅拷贝则是拷贝引用.
>
> ### 浅拷贝的实现
>
> > 首先我们要对传入的对象做一个判断,如果是基本类型,那么就直接返回即可(注意如果使用`typeof`去判断`null`得到的将会是`"object"`,因此`null`单独做判断),如果判断完不是基本类型,那么我们就开始使用 for(... in ...)来做一个遍历,并直接将目标值赋值给返回对象
>
> ### 深拷贝的实现
>
> > 相比于浅拷贝我们需要多注意两个点:
> >
> > 1. 深拷贝面对引用类型的值时不能直接赋值,而是要开辟新的内存空间,因此一般需要使用递归的形式
> > 2. 深拷贝时需要注意循环引用的问题,一般使用 Map 解决.
